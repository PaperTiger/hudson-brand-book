#!/usr/bin/env node
// Regenerates the 5 HCNJ horizontal-logo color variants (SVG + PNG) from the
// new master artwork, replacing images/logos/hcnj_horizontal-logo_*.{svg,png}
// Usage: node generate-hcnj-horizontal-logo-variants.js

const fs   = require('fs');
const path = require('path');
const { chromium } = require('playwright-core');

const masterPath = path.join(__dirname, 'Hudson County', 'HCNJ - Full', 'HCNJ logo full.svg');
const outDir     = path.join(__dirname, 'images', 'logos');
const stem       = 'hcnj_horizontal-logo';
...
// Master artwork is fill="black" on every visible path — replace with each
// brand color. The lone fill="white" is a <clipPath> rect in <defs>, which is
// invisible geometry-only; leaving it untouched is correct.
const variants = [
...
(async () => {
  ...
  await browser.close();
  console.log('\nDone — 5 HCNJ horizontal logo variants regenerated.');
})();
