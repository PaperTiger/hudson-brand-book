#!/usr/bin/env node
// Regenerates the 5 HCNJ text-lockup-logo color variants (SVG + PNG) from the
// new master artwork, replacing images/logos/hcnj_text-lockup-logo_*.{svg,png}
// Usage: node generate-hcnj-text-lockup-logo-variants.js

const fs   = require('fs');
const path = require('path');
const { chromium } = require('playwright-core');

const masterPath = path.join(__dirname, 'Hudson County', 'HCNJ - Text', 'HCNJ logo no seal.svg');
const outDir     = path.join(__dirname, 'images', 'logos');
const stem       = 'hcnj_text-lockup-logo';
...
(async () => {
  ...
  await browser.close();
  console.log('\nDone — 5 HCNJ text-lockup logo variants regenerated.');
})();
