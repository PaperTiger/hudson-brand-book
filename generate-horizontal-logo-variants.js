#!/usr/bin/env node
// Regenerates the 5 horizontal-logo color variants (SVG + PNG) from the new
// master artwork, replacing images/logos/hudson-county_full-logo_horizontal_*.{svg,png}
// Usage: node generate-horizontal-logo-variants.js

const fs   = require('fs');
const path = require('path');
const { chromium } = require('playwright-core');

const masterPath = path.join(__dirname, 'Hudson County', 'HC - Logo full', 'Logo-full.svg');
const outDir     = path.join(__dirname, 'images', 'logos');
const stem       = 'hudson-county_full-logo_horizontal';
...
(async () => {
  ...
  await browser.close();
  console.log('\nDone — 5 horizontal logo variants regenerated.');
})();
